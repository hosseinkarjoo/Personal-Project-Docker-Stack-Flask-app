...
data "template_file" "outputs" {
    template = "${file("./templates/outputs")}"
    vars = {
      regAddr = "${aws_ecr_repository.app.repository_url}"
    }
}
resource "null_resource" "outputs" {
  triggers = {
    template_rendered = "${ data.template_file.outputs.rendered }"
  }
  provisioner "local-exec" {
    command = <<-EOT
      echo '${ data.template_file.outputs.rendered }' > ./outputs 
